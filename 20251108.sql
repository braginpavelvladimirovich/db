...
/* Использование полей исходной таблицы в предложении HAVING не имеет смысла, так как сравнение происходит после группировки, значения в полях могут быть уже недоступны
SELECT 
    gamer, MAX(`time`)
FROM
    gamers_games
...
#Отбор при помощи HAVING и значений, полученных не агрегатными функциями, допускается в том случае, если после группировки у нас получились значения в колонках, используемые в условии 
SELECT 
    gamer, `date`, MAX(`time`)
FROM
    gamers_games
...
/* Недопустимо использовать вложенные агрегатные функции
SELECT 
    gamer, `date`, AVG(MAX(`time`))
FROM
    gamers_games
GROUP BY gamer , `date`;
*/